#!/usr/bin/env python3
"""
Teste da lógica do gerenciamento Torre MK com 5 wins por nível
"""
...
def test_gerenciamento_5wins():
    # Configuração de teste - 5 wins para subir de nível
    config = {
    ...
    print("=== TESTE DO GERENCIAMENTO COM 5 WINS POR NÍVEL ===")
    print(f"Banca inicial: ${banca_inicial}")
    print(f"Entrada nível 1: ${gerenciador.get_proxima_entrada()}")
    print()
    
    # Teste 1: Subir para nível 2 (5 wins)
    print("--- TESTE 1: Subir para nível 2 (5 wins) ---")
    for i in range(5):
        ...
        print(f"Win {i+1}: Entrada ${entrada}, Nível {nivel}, Wins no nível: {wins_no_nivel}")
    ...
    print(f"Após 5 wins: Entrada ${entrada_lvl2}, Nível {nivel_atual}")
    print()
    
    # Teste 2: Subir para nível 3 (mais 5 wins)
    print("--- TESTE 2: Subir para nível 3 (mais 5 wins) ---")
    for i in range(5):
        entrada = gerenciador.get_proxima_entrada()
        nivel = gerenciador._get_level_from_wins(gerenciador.total_wins)
        wins_no_nivel = gerenciador.total_wins % config['wins_to_level_up']
    ...
    nivel_atual = gerenciador._get_level_from_wins(gerenciador.total_wins)
    print(f"Após mais 5 wins: Entrada ${entrada_lvl3}, Nível {nivel_atual}")
    print()
